...
proc newStringClaim*(s: string): Claim =
    ## Create a general string claim
    return newClaim(GENERAL, %s)

proc newIntClaim*(i: int64): Claim =
    ## Create a general integer claim
    return newClaim(GENERAL, %i)

proc newBoolClaim*(b: bool): Claim =
    ## Create a general boolean claim
    return newClaim(GENERAL, %b)

proc newJsonClaim*(j: JsonNode): Claim =
    ## Create a general JSON claim
    return newClaim(GENERAL, j)

proc newTimeClaim*(t: Time): Claim =
    ## Create a time claim from Time object (for iat, exp, nbf)
    return newClaim(GENERAL, %t.toUnix)
